...
function md5(input: string): string {
	...
	for (let i = 0; i < 64; i++) {
		...
		d = c;
		c = b;
		b = ADD32(b, ROL(t, S[(i >> 4) | ((i & 3) << 2)]));
	}
	a = ADD32(a, 1732584193);
	b = ADD32(b, -271733879);
	c = ADD32(c, -1732584194);
	d = ADD32(d, 271733878);
...
function keccak256(input: string): string {
	const RC = [
		0x01, 0x8082, 0x808a, 0x8000, 0x808b, 0x0001, 0x8081, 0x8009, 0x008a,
	...
	const lanes: bigint[][] = Array.from({ length: 5 }, () =>
		Array(5).fill(0n)
	);
