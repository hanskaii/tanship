...
export class ChatAgent extends AIChatAgent<CloudflareBindings> {
	async onChatMessage(
		...
		// 1. Validate Balance
		const { credits } = await this.getUserInfo();
		if (credits !== null && credits <= 0) {
			this.broadcast(
				JSON.stringify({
					type: "error",
					message:
						"Insufficient balance. Please top up your credits to continue."
				})
			);
			return;
		}
	...
	${getSchedulePrompt({ date: Day().toDate() })}
...
If the user asks to schedule a task, use the schedule tool to schedule the task.`,
			...
			onFinish: async (event: any) => {
				if (onFinish) await (onFinish as any)(event);
			},
	...
	/**
	 * Resolves user info (Dodo Customer ID & current credits) for the session.
	 * This is typically passed via metadata during agent initialization.
	 */
	private async getUserInfo(): Promise<{
		...
		return {
			dodoCustomerId: metadata?.dodoCustomerId ?? null,
