		...
		<div className="relative flex min-h-screen flex-col items-center bg-background selection:bg-primary/20 overflow-hidden">
			...
			<main className="relative z-10 mt-28 mb-32 flex w-full max-w-5xl flex-col items-center px-6">
				...
				>
					...
					<div
						className={`grid w-full gap-6 sm:grid-cols-2 max-w-7xl ${{ 1: "lg:grid-cols-1", 2: "lg:grid-cols-2", 3: "lg:grid-cols-3", 4: "lg:grid-cols-4" }[appConfig.payments.filter((p: any) => !p.slug.startsWith("template-")).length as 1 | 2 | 3 | 4] ?? "lg:grid-cols-3"}`}
					>
						{appConfig.payments
